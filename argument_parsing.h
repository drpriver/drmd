...
static inline
enum ArgParseError
parse_args_strings(ArgParser* parser, const StringView* longstrings, size_t count, enum ArgParseFlags);
...
print_argparse_help(const ArgParser* p, int columns){
    ...
    for(size_t i = 0; i < p->positional.count; i++){
        ...
        if(arg->max_num > 1){
            ...
            help_state_update(&hs, (int)to_print);
...
print_wrapped(const char*text, int columns){
    ...
    for(;*text;){
        ...
        help_state_update(&hs, (int)tok.token.length);
...
static inline size_t agp_maxnum(int x){ return x?x:1; }
...
parse_arg(ArgToParse* arg, StringView s){
    ...
    if(arg->num_parsed >= (size_t)agp_maxnum(arg->max_num))
...
#define APPEND_ARG(type, value_) do { \
    if(arg->append_proc){ \
        type tmp = value_; \
        int fail = arg->append_proc(arg->dest.pointer, &tmp); \
...
parse_args(ArgParser* parser, const Args* args, enum ArgParseFlags flags){
    ...
    for(size_t i = 0; i < parser->positional.count; i++){
        ...
        if(arg->num_parsed < (size_t)arg->min_num){
...
static inline
enum ArgParseError
parse_args_strings(ArgParser* parser, const StringView*args, size_t args_count, enum ArgParseFlags flags){
    ArgToParse* pos_arg = NULL;
    ArgToParse* past_the_end = NULL;
    if(parser->positional.count){
        pos_arg = &parser->positional.args[0];
...
print_argparse_error(ArgParser* parser, enum ArgParseError error){
    ...
    switch(error){
        ...
        case ARGPARSE_EXCESS_ARGS:{
            ...
            if(!parser->failed.arg){
                fprintf(stderr, "Excess arguments. No more than %zu arguments needed. Unknown first excess argument (this is a bug)\n", agp_maxnum(arg_to_parse->max_num));
                return;
            }
            fprintf(stderr, "Excess arguments. No more than %zu arguments needed. First excess argument: '%s'\n", agp_maxnum(arg_to_parse->max_num), parser->failed.arg) ;
        }return;
